...
class Ui_MainWindow(object):
    ...
    def selectFile(self):
        # 前处理
        if os.path.exists("input/example.png"):
            os.remove("input/example.png")
        if os.path.exists("input/example2.png"):
            os.remove("input/example2.png")
        self.img_path = ""
        ...
        if self.img_path == "":
            ...
            try:
                ...
                shutil.copy(self.img_path, "input/example.png")
                outfile, get_size = self.compress_image(r"D:\Code\projects\linedis\input\example.png",
                                                   r"D:\Code\projects\linedis\input\example2.png")
                self.resize_image(r"D:\Code\projects\linedis\input\example.png",x_s=200)
                self.label.setPixmap(QtGui.QPixmap("input/example2.png"))
                print(self.img_path)
    ...
    def predict(self):
        try:
            if os.path.exists("output/example.png"):
                os.remove("output/example.png")
            if os.path.exists("output/example2.png"):
                os.remove("output/example2.png")
            model = load_model('model.h5')
            img = cv2.imread("input/example.png")  # 从内存数据读入图片
            ...
            cv2.imencode(".jpg", img_res)[1].tofile(os.path.join('./output', "".join(
                [self.img_path.split("/")[-1].replace(self.img_path.split("/")[-1].split(".")[-1], ""), "jpg"])))
            # 处理文件
            shutil.copy(os.path.join('./output', "".join(
                [self.img_path.split("/")[-1].replace(self.img_path.split("/")[-1].split(".")[-1], ""), "jpg"])),
                        "output/example.png")
            outfile, get_size = self.compress_image(r"D:\Code\projects\linedis\output\example.png",
                                               r"D:\Code\projects\linedis\output\example2.png")
            self.resize_image(r"D:\Code\projects\linedis\output\example.png",x_s=200)
            self.label_2.setPixmap(QtGui.QPixmap("output/example2.png"))
    ...
    def method1(self):
        ...
        path = "D:\Code\projects\linedis\process"
        # print(os.listdir(path))
        i = 1
        for file in os.listdir(path):
            print(file)
            file_path = os.path.join(path, file)
            dict1["s" + i.__str__()] = file.split(".")[0]
    ...
    def main(self):
        ...
        n = ""
        try:
            for root, dirs, files in os.walk(os.getcwd() + '\\process', topdown=False):
                for name in files:
                    r = root
                    n = name
                    ...
                    s2 = name.replace(s1, "")
                    s3 = s2[0:len(s2) - 1]
                    new_name = dict1.get(s3)
                    print(new_name)
                    cv2.imencode(".jpg", im_res)[1].tofile(os.path.join('./output', ".".join([new_name, "jpg"])))
...
if __name__ == '__main__':
    ...
    # D:\SoftWare\Anaconda3\envs\linedis\Lib\site-packages\PyQt5\Qt5\bin
    # D:\Code\projects\linedis\sys.py